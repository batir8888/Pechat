...
class TypingSimulator:
    ...
    def setup_ui(self):
        ...
        # Конвертация пробелов в табы
        self.convert_spaces = tk.BooleanVar(value=True)
        ttk.Checkbutton(settings_frame, text="Конвертировать 4 пробела в табуляцию",
                        variable=self.convert_spaces).grid(row=0, column=0, sticky=tk.W)

        # Настройки скорости
        speed_frame = ttk.Frame(settings_frame)
        speed_frame.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=5)
        ...
        delay_frame.grid(row=2, column=0, sticky=tk.W, pady=5)
    ...
    def convert_spaces_to_tabs(self, text):
        """Конвертирует 4 пробела в табуляцию"""
    ...
    def typing_worker(self):
        ...
        try:
            ...
            # Конвертируем пробелы в табы если нужно
            text = self.convert_spaces_to_tabs(text)
